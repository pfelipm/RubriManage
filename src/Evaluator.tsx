...
export const Evaluator: React.FC<EvaluatorProps> = ({ rubric, group, onClose }) => {
  ...
  if (loading) return <div className="flex justify-center p-20">Cargando evaluaciones...</div>;

  return (
    <div className="max-w-7xl mx-auto space-y-6">
      {/* Header / Navigation */}
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-black/5 flex flex-wrap items-center justify-between gap-4 sticky top-0 z-10">
        <div className="flex items-center gap-4">
          <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-full transition-colors">
            <ChevronLeft className="w-6 h-6" />
          </button>
          <div>
            <h2 className="text-lg font-bold text-gray-900">{rubric.title}</h2>
        ...
        <div className="flex items-center gap-4 bg-gray-50 p-2 rounded-xl border">
          <button onClick={prevStudent} disabled={currentStudentIndex === 0} className="p-1.5 hover:bg-white disabled:opacity-30 rounded-lg shadow-sm transition-all">
          ...
          <button onClick={nextStudent} disabled={currentStudentIndex === sortedStudents.length - 1} className="p-1.5 hover:bg-white disabled:opacity-30 rounded-lg shadow-sm transition-all">
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            <div className="text-xs text-gray-500 uppercase tracking-wider font-bold">Calificación</div>
            <div className={cn(
              "text-3xl font-black",
              displayScore >= (rubric.maxScore / 2) ? "text-emerald-600" : "text-rose-600"
            )}>
              {displayScore.toFixed(2)}
              <span className="text-sm text-gray-400 font-normal ml-1">/ {rubric.maxScore}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-black/5">
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            <thead>
              <tr>
                <th className="p-6 text-left bg-gray-50/50 border-b w-64 font-bold text-gray-400 uppercase text-xs tracking-widest">Indicador</th>
                {rubric.indicators[0]?.levels.map((level, i) => (
                  <th key={i} className="p-6 text-center bg-gray-50/50 border-b font-bold text-gray-400 uppercase text-xs tracking-widest whitespace-normal min-w-[120px]">
                    {level.name}
            ...
            <tbody>
                ...
                <tr key={indicator.id || i} className="group hover:bg-gray-50/30 transition-colors">
                  <td className="p-6 border-b border-gray-100 align-top">
                    <div className="font-bold text-gray-900 mb-1">{indicator.name}</div>
                    <div className="text-xs text-gray-400 italic">Peso: {indicator.weight}</div>
                  </td>
                  {indicator.levels.map((level, j) => {
                    ...
                    return (
                      <td 
                        ...
                        className={cn(
                          "p-6 border-b border-gray-100 cursor-pointer transition-all relative",
                          isSelected 
                            ? "bg-indigo-50/80 ring-2 ring-indigo-500/20 z-10" 
                      ...
                      >
                        ...
                        <div className={cn(
                          ...
                          isSelected ? "text-indigo-900 font-medium" : "text-gray-600"
                        )}>
                          {level.description || <span className="text-gray-300 italic">Sin descripción</span>}
                        </div>
                      </td>
                    );
                  })}
      ...
      {/* Summary / Progress */}
      <div className="flex justify-between items-center p-6 bg-gray-900 text-white rounded-3xl shadow-2xl">
